...
import type { ChatRequest, ChatResponse, BookSuggestion } from "./types";
...
function buildTool(name: string, description: string, properties: Record<string, JsonSchema>, required: string[]) {
  return {
...
function buildRemoveFromBoardTool() {
  return buildTool(
    "removeFromBoard",
...
export async function handleChat(req: Request, res: Response) {
  ...
  if (!body.settings?.apiKey) {
    res.status(400).json({ error: "Missing API key — check Settings" });
    return;
  }
  if (!body.settings?.model) {
    res.status(400).json({ error: "No model selected — check Settings" });
    return;
  }
  ...
  try {
    ...
    const messages: OpenAI.Chat.ChatCompletionMessageParam[] = buildMessages(body);
    ...
    for (let i = 0; i < MAX_TOOL_ITERATIONS; i++) {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages,
        tools,
        ...(isLastIteration ? { tool_choice: { type: "function", function: { name: "addToBoard" } } } : {}),
      });
    ...
    if (finalSuggestions.length === 0) {
      ...
      if (toolCall?.function.name === "addToBoard") {
        ...
        if (!finalText && response.choices?.[0]?.message?.content) {
    ...
    if (finishedTitles.length > 0) {
      ...
      // also add them to the removed list so the frontend can track them
    ...
    if (message.includes("429") || message.includes("rate")) {
      ...
      res.status(401).json({ error: "Invalid API key — update in Settings" });
    } else {
